...
export const init = (app) => {
    ...
    app.use('/model.json',
        falcorExpress.dataSourceRoute((req, res) => {
            ...
            return {
                get: function (paths) {
                    logger.silly(`${req.user._id} requested paths ${JSON.stringify(paths)}`);
                    return routerInstance.get(paths)
                },
                set: function (jsong) {
                    logger.silly(req.user._id + ' set the following jsong ' + JSON.stringify(jsong));
                    return routerInstance.set(jsong)
                },
                call: function (callPath, args, suffixes, paths) {
                    logger.silly(req.user._id + ` made a call to ${callPath} with the arguments ${JSON.stringify(args)}`)
